import streamlit as st
import re

try:
    import fitz  # PyMuPDF for line-preserving extraction
    PYMUPDF_AVAILABLE = True
except Exception:
    PYMUPDF_AVAILABLE = False

st.set_page_config(layout="wide")
st.title("Reading Practice App")

uploaded = st.file_uploader("Upload your PDF", type=["pdf"])

def extract_lines(file_bytes: bytes):
    """Extract text lines using PyMuPDF."""
    doc = fitz.open(stream=file_bytes, filetype="pdf")
    all_lines = []
    for pno in range(len(doc)):
        page = doc[pno]
        data = page.get_text("dict")
        for block in data.get("blocks", []):
            if "lines" not in block:
                continue
            for line in block["lines"]:
                spans = line.get("spans", [])
                if not spans:
                    continue
                line_text = "".join(span.get("text", "") for span in spans).rstrip("\n")
                if line_text is None:
                    continue
                all_lines.append(line_text)
    return all_lines

def split_passage_questions(lines, split_key="QUESTIONS"):
    """Split lines into passage vs questions."""
    q_index = None
    for i, ln in enumerate(lines):
        if ln.strip().lower().startswith(split_key.lower()):
            q_index = i
            break
    if q_index is None:
        return lines, []
    return lines[:q_index], lines[q_index:]

def group_text_sections(lines):
    """Group into Text A, B, C, D sections for display."""
    sections = []
    current_title = "Passage"
    buf = []
    for ln in lines:
        if re.match(r"^\s*Text\s+[A-D]\b", ln, flags=re.IGNORECASE):
            if buf:
                sections.append((current_title, buf.copy()))
                buf.clear()
            current_title = ln.strip()
        else:
            buf.append(ln)
    if buf:
        sections.append((current_title, buf))
    return sections

def parse_numbered_questions(question_lines):
    """Parse numbered questions (with instructions intact)."""
    questions_text = "\n".join(question_lines)
    parts = re.split(r"(?m)^\s*(\d{1,2})\.\s+", questions_text)
    q = {}
    preamble = parts[0].strip()
    if preamble:
        q["intro"] = preamble
    for i in range(1, len(parts)-1, 2):
        num = parts[i]
        body = parts[i+1].strip()
        q[int(num)] = body
    return q

if uploaded:
    if not PYMUPDF_AVAILABLE:
        st.error("PyMuPDF not installed. Run `pip install pymupdf`")
    else:
        raw = uploaded.read()
        lines = extract_lines(raw)
        passage_lines, question_lines = split_passage_questions(lines, "QUESTIONS")

        col1, col2 = st.columns(2)

        with col1:
            st.subheader("Reading Passage")
            sections = group_text_sections(passage_lines)
            for title, seg_lines in sections:
                with st.expander(title, expanded=True):
                    st.text_area("", "\n".join(seg_lines), height=300, disabled=True)

        with col2:
            st.subheader("Questions")
            qs = parse_numbered_questions(question_lines)
            if "intro" in qs:
                st.markdown(qs["intro"])
            for k in sorted([x for x in qs.keys() if isinstance(x, int)]):
                st.markdown(f"**{k}. {qs[k]}**")
                st.text_input(f"Answer {k}", key=f"ans_{k}")
